...
- CRM 负责业务入口、权限、审计、客户关联、通话记录、录音回看和 AI 质检结果展示
- PBX / Asterisk / SIP provider 负责线路注册、坐席桥接、服务端录音和 CDR 采集
- 外呼入口在客户详情页，配置入口在 `/settings/outbound-call`
- 录音存储配置入口在 `/settings/recording-storage`
- 录音 AI 配置入口在 `/settings/call-ai`
- 录音质检主入口是 `/call-recordings`
- 正式录音以服务端录音为准，不依赖浏览器或手机本地保存
- `OutboundCallSession` 记录外呼会话状态，`CallRecord` 记录客户通话业务事实，`CallRecording` 记录录音文件，`CallAiAnalysis` 记录转写与质检分析
- 员工应能看到外呼是否已接通、通话时长和对应通话记录；主管以上可在录音质检页查看团队录音与 AI 分析
- AI 质检是辅助判断，不替代主管复核和业务审计
- 生产推荐 ASR 使用 `gpt-4o-transcribe-diarize`，LLM 使用 DeepSeek `deepseek-v4-pro`
...
- 可发起并追踪团队线索导入批次
- 可查看团队录音质检和 AI 分析结果
- 可在客户详情中移交客户负责人，并保留 ownership 审计链
...
- 提交支付记录
- 跟进自己的催收与物流结果
- 使用客户详情页外呼自己的客户，并查看自己的通话记录和通话结果
- 不默认获得团队级导入批次处理视图
- 不默认获得客户负责人移交权限
