...
| 类别 | 数量 | 说明 |
|---|---:|---|
| 提交 | 17 | 全部按"事务边界分组", 风险隔离 |
| 部署 | 17 | 每次 commit 后跑 release-preflight + migrate deploy + smoke |
...
**线上 HEAD**: `4379d2e` (含 11 轮迭代 — 跨模块 cascade 隐藏债已 catch)
...
## Round 11: 跨模块 vector check (`4379d2e`) ⚠️ 关键

**Bug**: 今晚新加的 `TradeOrderRevisionRequest` 表跟 `TradeOrder` 是 `ON DELETE RESTRICT` FK. 任何 hard-delete TradeOrder 的路径未先清 revision 会 FK 违约整个事务回滚 — 用户会发现"含 revision 历史的客户/订单都不能强删".

修了 2 条 hard-delete 路径(grep 确认只这 2 条):

1. `lib/customers/force-delete.ts` `forceHardDeleteCustomer`(主管硬删客户)
2. `lib/recycle-bin/trade-order-adapter.ts` `purgeTradeOrderTarget`(回收站 30 天后真删)

**关键认知 (写入 NIGHT_SUMMARY 给将来 reference)**: schema 新增引入的 cascade 债**不在任何"看 diff"型 audit/review 的范围内**. Round 1 全仓 audit 看 existing code, Round 7 agent review 看今晚 diff, Round 8-9 自审看 trade-orders 内逻辑 — 这三种都不会主动反向问"我新加的这张表跟谁有 FK, 这些 FK 的所有 hard-delete 调用方都处理了吗?". 必须从 schema 反扫所有 `tradeOrder.delete` 路径才能 catch.

**这是这个夜晚最有价值的 catch** — 上线后用户白天若强删一个有撤单历史的客户, 会直接 500 错误, 大概率没人能 debug 到 FK constraint 上.
...
| e306c54 | test checklist + 本次 README 更新 | 11:xx |
| **4379d2e** | **跨模块 vector check — force-delete + recycle cascade fix** | (本提交) |
