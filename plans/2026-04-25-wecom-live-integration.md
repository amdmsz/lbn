...
- preserve RBAC（服务端权限）and `OperationLog` auditability
...
- Add a detail / expandable panel for audience records and a “待员工确认” queue.
...
- Add copy: “真实进场人数依赖 WeCom 返回的可识别用户；未匹配观众不会自动变成客户。” - Add copy: “手机号 / 昵称会进入待确认队列；员工确认后才会写入客户直播记录。”
### 8. RBAC and audit
