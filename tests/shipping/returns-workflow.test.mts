...
test("反例: PENDING_RETURN_TRACKING 直接入库, 未显式勾选无运单 → throw (防丢运单)", async () => {
  resetStore();
  seedHappyPath();
  const sr = await requestShippingReturn(SALES, {
    tradeOrderId: "to_1",
    shippingTaskId: "st_1",
    reason: "WRONG_ITEM",
    reasonDetail: "想跳过运单直接入库, 但没开 receivedWithoutTracking",
  });
  await reviewShippingReturn(SUPERVISOR, {
    shippingReturnId: sr.id,
    decision: "APPROVED",
  ...
  // 状态不应被推进
  assert.equal(
    store.shippingReturns[0]!.status,
...
test("反例: 勾选了 receivedWithoutTracking 但理由不足 4 字 → throw", async () => {
  resetStore();
  seedHappyPath();
  const sr = await requestShippingReturn(SALES, {
    tradeOrderId: "to_1",
  ...
  await assert.rejects(
    () =>
      confirmShippingReturnReceived(SHIPPER, {
        shippingReturnId: sr.id,
        receivedWithoutTracking: true,
        receivedWithoutTrackingReason: "无",
      }),
    /无运单入库必须填写至少 4 个字的理由/,
  );
});

test("发货人可从 PENDING_RETURN_TRACKING 现场签收入库 (显式勾选 + 理由) → RETURNED_TO_WAREHOUSE + 留无运单 audit log", async () => {
  resetStore();
  seedHappyPath();
  const sr = await requestShippingReturn(SALES, {
    tradeOrderId: "to_1",
  ...
  const result = await confirmShippingReturnReceived(SHIPPER, {
    shippingReturnId: sr.id,
    receivedWithoutTracking: true,
    receivedWithoutTrackingReason: "客户上门自取, 司机无运单回执",
  });
  assert.equal(result.shippingReturn.status, "RETURNED_TO_WAREHOUSE");
  // 自动退款已建
  assert.equal(store.refundRequests.length, 1);

  // 必须落 received_without_tracking 审计日志, 带原因 — 财务后续核对凭据
  const skipLog = store.operationLogs.find(
    (l) => l.action === "shipping_return.received_without_tracking",
  );
  assert.ok(skipLog, "必须写 shipping_return.received_without_tracking 审计日志");
  const after = skipLog!.afterData as Record<string, unknown>;
  assert.equal(after.reason, "客户上门自取, 司机无运单回执");
  assert.equal(after.previousStatus, "PENDING_RETURN_TRACKING");
});

test("IN_RETURN_TRANSIT 入库时 receivedWithoutTracking 不应触发, 不写无运单日志", async () => {
  resetStore();
  seedHappyPath();
  const sr = await requestShippingReturn(SALES, {
    tradeOrderId: "to_1",
    shippingTaskId: "st_1",
    reason: "QUALITY_ISSUE",
    reasonDetail: "IN_RETURN_TRANSIT 路径不该被新守卫影响",
  });
  await reviewShippingReturn(SUPERVISOR, {
    shippingReturnId: sr.id,
    decision: "APPROVED",
  ...
  await fillShippingReturnTracking(SHIPPER, {
    shippingReturnId: sr.id,
    returnTrackingNumber: "SF8888888",
    returnCarrier: "顺丰",
  });
  // 即使误传 receivedWithoutTracking, 服务端识别 status=IN_RETURN_TRANSIT 应忽略
  const result = await confirmShippingReturnReceived(SHIPPER, {
    shippingReturnId: sr.id,
    receivedWithoutTracking: true,
    receivedWithoutTrackingReason: "误传也不该触发",
  });
  assert.equal(result.shippingReturn.status, "RETURNED_TO_WAREHOUSE");
  assert.equal(
    store.operationLogs.some(
