...
export function FinanceSubnav({
  ...
  return (
    <nav className="rounded-xl border border-[var(--color-border-soft)] bg-[var(--color-panel-soft)] px-1 py-1 shadow-[var(--color-shell-shadow-sm)]">
      <div className="flex flex-wrap items-center gap-1">
        {financeTabs.map((tab) => (
          <Link
            ...
            href={tab.href}
            className={cn(
              "inline-flex min-h-8 items-center rounded-xl border px-3 py-1.5 text-[12.5px] font-medium transition-[border-color,background-color,box-shadow,color,transform] duration-150 motion-safe:hover:-translate-y-[1px]",
              active === tab.key
