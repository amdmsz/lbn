...
function getTranslatedIssueMessage(
  ...
  const lineLabel = issue.lineId
    ? `第 ${lineIndexById.get(issue.lineId) ?? "-"} 行`
    : "订单";
...
export function TradeOrderForm({
  ...
  const lineIndexById = new Map(lines.map((line, index) => [line.lineId, index + 1] as const));
  const issueMessages = computation.issues.map((issue) =>
    getTranslatedIssueMessage(issue, lineIndexById),
  );
  ...
  for (const issue of computation.issues) {
    ...
    existing.push(getTranslatedIssueMessage(issue, lineIndexById));
  ...
  const canRemoveSkuLine = lines.length > 1;
  ...
  function removeLine(lineId: string) {
    setLines((current) =>
      current.length > 1 ? current.filter((item) => item.lineId !== lineId) : current,
    );
  }
  ...
  return (
    <form className="space-y-4">
      ...
      />
      <input type="hidden" name="giftLinesJson" value={JSON.stringify([])} />
      ...
      <section className="overflow-hidden rounded-[1.25rem] border border-border/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(247,249,252,0.9))] shadow-[var(--color-shell-shadow-sm)]">
        ...
        <div className="grid gap-3 p-3.5 md:grid-cols-2 md:p-4 xl:grid-cols-4">
          ...
          <MetricTile
            label="商品行 / 件数"
            value={`${computation.totals.skuLineCount} / ${computation.totals.qtyTotal}`}
            note={`优惠 ${formatCurrency(computation.totals.discountAmount)}`}
          />
          <MetricTile
            label="执行分组"
          ...
          <section className="overflow-hidden rounded-[1.15rem] border border-border/60 bg-card shadow-sm">
            ...
            <div className="space-y-4 p-4">
              <div className="rounded-2xl border border-primary/15 bg-primary/5 px-4 py-4">
                ...
                <p className="mt-1 text-[12px] leading-5 text-muted-foreground">
                  {selectedPaymentScheme?.label ?? "未选择支付方案"} / 商品行{" "}
                  {computation.totals.skuLineCount}
                </p>
              </div>
