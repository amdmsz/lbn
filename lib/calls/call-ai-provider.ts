...
function buildAnalysisPrompt(input: {
  ...
  return [
    ...
    "目标不是泛泛总结，而是给销售和主管可执行的复盘信号：先判断业务场景，再指出客户真实意向、销售动作缺口、风险、机会、关键词和下一步动作。",
    "",
    ...
    "先在内部判断场景，只能从这些场景里选最贴近的一个：NEW_LEAD_ADD_WECHAT=新线索首呼，核心目标是加微信；LIVE_INVITE_CONVERSION=直播邀约/直播攻单，核心目标是到场、下单或付款；EXISTING_CUSTOMER_FOLLOWUP=老客户复购或常规跟进；AFTER_SALES_OR_OTHER=售后、物流、杂项或信息不足。",
    "summary 用 2 到 4 句中文，第一句必须写清场景判断和依据；后续按顺序覆盖：客户需求/问题、客户态度、销售回应、当前成交或跟进结论。短电话也要指出信息不足。",
    ...
    "qualityScore 是 0-100 或 null。必须按场景评分：NEW_LEAD_ADD_WECHAT=身份与信任 15，加微信理由/利益点 25，需求信息收集 20，异议处理 15，微信承接或回访动作 15，合规边界 10；LIVE_INVITE_CONVERSION=直播利益点 20，需求/产品/套餐匹配 20，成交推进 25，价格/品质/物流异议处理 15，直播承接动作 10，合规边界 10；其他场景再用通用口径：开场信任 15，需求挖掘 20，匹配 20，异议处理 15，下一步 20，合规 10。没有有效销售对话时必须低分。",
    "riskFlags 输出最多 12 个中文短标签。必须优先指出场景关键缺口，例如：未说明加微信理由、未给微信价值、未确认微信承接、未预约直播、未讲直播利益点、未推动下单、未处理价格异议、客户拒绝、话术过短、信息不足、需主管复核。没有风险则输出 []。",
    "opportunityTags 输出最多 12 个中文短标签。第一项尽量写场景标签，例如：场景:新线索加微信、场景:直播攻单、场景:老客跟进、场景:售后其他。其他标签可包含：可加微信、可发酒品资料、可直播提醒、可推直播套餐、询问价格、询问品质、有送礼需求、有宴请场景、可二次回访。没有机会则输出 []。",
    "keywords 输出最多 20 个中文关键词，优先提取酒品、价格、规格、直播、微信、用途、预算、竞品、异议和客户关注点。",
    "nextActionSuggestion 用一句中文给出明确动作。NEW_LEAD_ADD_WECHAT 必须给出下一句加微信/补发资料/回访话术；LIVE_INVITE_CONVERSION 必须给出直播提醒、套餐推进、订单或付款承接动作；信息不足时建议补打确认。",
