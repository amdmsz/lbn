...
- `npm run db:reconcile-prisma-names`
...
- `prisma:name-drift` 会在 `lower_case_table_names=0` 的 MySQL 上检查物理表名、索引名、外键名是否与 `schema.prisma` 大小写一致。
- `db:reconcile-prisma-names` 只允许执行 `RENAME INDEX`、`DROP FOREIGN KEY`、`ADD FOREIGN KEY` 这类 name-only SQL；默认 dry-run，生产执行 `--apply` 前必须先备份。
...
## 5. 大小写敏感 MySQL 的物理命名漂移处理

Linux MySQL 常见 `lower_case_table_names=0`，表名、索引名、外键名会出现大小写敏感问题。历史库如果曾经使用过 Prisma 默认模型名表，例如 `User`、`Customer`、`OperationLog`，而当前 schema 使用 `@@map("user")`、`@@map("customer")`、`@@map("operationlog")`，正式发布时会出现 `Table ... doesn't exist` 或 `Duplicate key name ...`。

发布前先跑：

```bash
ENV_FILE=/etc/jiuzhuang-crm/jiuzhuang-crm.env npm run prisma:name-drift
```

如果发现 drift，处理顺序固定为：

```bash
ENV_FILE=/etc/jiuzhuang-crm/jiuzhuang-crm.env \
BACKUP_DIR=/var/www/jiuzhuang-crm/backups/mysql \
bash scripts/backup-mysql.sh

systemctl stop jiuzhuang-crm || true
systemctl stop jiuzhuang-crm-import-worker || true
...
ENV_FILE=/etc/jiuzhuang-crm/jiuzhuang-crm.env npm run prisma:predeploy:check
```

只有 `predeploy:check` 返回 `No difference detected` 后，才能继续正式发布。不要直接跳过 guardrails，也不要手工粘贴未审计 SQL。

## 6. baseline 重建何时需要，何时不能贸然做
...
3. `npm run prisma:name-drift`
4. `npm run prisma:diff:schema`
5. 如已配 shadow database，再跑 `npm run prisma:diff:migrations`
6. 检查最近是否有手工 SQL 热修但未回填
7. 最后才讨论是否进入 baseline 重建方案
