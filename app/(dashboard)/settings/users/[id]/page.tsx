  ...
  return (
    <div className="crm-page">
      <PageHeader
        ...
        description="查看账号的团队归属、直属主管、启停状态和审计记录，并在允许的范围内完成信息维护、临时密码重置与启停。"
        actions={
          <>
            <StatusBadge
              label={roleLabels[data.user.role.code]}
              variant={getRoleBadgeVariant(data.user.role.code)}
            />
            <StatusBadge
              label={getUserStatusLabel(data.user.userStatus)}
              variant={getUserStatusVariant(data.user.userStatus)}
            />
            <StatusBadge
      ...
      <SectionCard
        title="账号概况"
        description="这里展示当前账号的团队、直属主管、启停记录和最近登录信息。"
      >
        <div className="grid gap-3.5 md:grid-cols-2 xl:grid-cols-3">
          ...
          <DetailItem
            ...
            value={
              data.user.supervisedTeam
                ? `${data.user.supervisedTeam.name} 的主管`
                : "当前账号未担任团队主管"
            }
          />
          <DetailItem
            label="创建时间"
            value={formatDateTimeLabel(data.user.createdAt)}
          />
          <DetailItem
            label="邀请创建时间"
            value={formatDateTimeLabel(data.user.invitedAt)}
          />
          <DetailItem
            label="邀请人"
            value={
          ...
          <DetailItem
            label="最近登录"
            value={formatDateTimeLabel(data.user.lastLoginAt)}
          />
          <DetailItem
            label="禁用时间"
            value={formatDateTimeLabel(data.user.disabledAt)}
          />
          <DetailItem
            label="禁用操作人"
            value={
            ...
            {data.operationLogs.map((item) => (
              <div key={item.id} className="crm-card-muted p-4">
                <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <p className="text-sm font-medium text-black/80">{item.description ?? item.action}</p>
